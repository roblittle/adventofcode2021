...
const winner = function(boardID, bingoBallVal){
    ...
    console.log('*')
    console.log(bingoBallVal);
    console.log('high score possible: ' + bingoBallVal*boardSum);
    return 'BINGO';
}

let allBoards = createAllBoards(inputArray);
let notLosingBoardID = -1;
for(bingoBall of bingoDraws){
    calledNumbers.push(bingoBall);

    for(let boardID=0; boardID<allBoards.length; boardID++){
        if(finishedBoards.length === allBoards.length){
            return;
        }
        ...
        if(boardResult === 'BINGO'){
            ...
            notLosingBoardID = boardID;
            finishedBoards.push(notLosingBoardID);
        }
    }
