...
const parseLineCoords = function(line){
    ...
    // - only horizontal or verticle lines aka x1 = x2 or y1 = y2
    ...
    if((lineStartX === lineStartY) || (lineEndX === lineEndY)){
        ...
        // - now handle diagonal lines
        while(lineStartX < lineStartY && lineEndX < lineEndY){
            // fill in x to y
            vectorArray.push([lineStartX,lineEndX].map(Number));
            lineStartX++;
            lineEndX++;
...
const counts = {};
vectorArray.forEach(function (x) { 
